...
function describeScore(score: number | null): ScoreDescriptor {
  ...
  if (score >= 0.7) {
    return {
      label: "Signal: High",
      summary:
        "You are a trusted, high-quality participant in the Farcaster graph.",
    };
  }
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      try {
        ...
        console.error(e);
        if (!cancelled) {
          setError("Unexpected error while loading your profile.");
        }
      } finally {
        if (!cancelled) {
          setIsLoading(false);
      ...
      <main className="flex min-h-screen items-center justify-center bg-background px-6">
        <div className="max-w-md rounded-3xl border border-white/10 bg-card/80 p-6 text-sm text-white/80">
          ...
          <h1 className="mt-3 text-lg font-semibold">
            Open inside Base App or Warpcast
          </h1>
          <p className="mt-2 text-sm text-white/60">
            This tool is designed to run as a Farcaster / Base mini app. Launch
            it from a cast, your app drawer, or a Base App link to see your
            live Neynar profile, followers, and score.
          </p>
        </div>
      </main>
    );
